...
export const findUnusedExports = defineTool({
  ...
  async handler(params, engine) {
    ...
    if (!symbols) return 'Could not get symbols for this file.';

    const currentPkg = getPackageName(params.file_path);
    const unused: { name: string; kind: string; line: number }[] = [];
    const KINDS: Record<number, string> = { 5: 'Class', 10: 'Enum', 11: 'Interface', 12: 'Function', 13: 'Variable', 14: 'Constant' };

    const checkSymbol = async (sym: DocumentSymbol) => {
      const lineText = lines[sym.range.start.line] ?? '';
      if (!lineText.includes('export')) return;
    ...
    if (unused.length === 0) return `✅ ${rel} — all exports are used cross-package.`;
