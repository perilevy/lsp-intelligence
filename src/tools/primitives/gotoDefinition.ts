...
export const gotoDefinition = defineTool({
  ...
  description:
    'Jump to where a symbol is defined. Accepts symbol name OR file position. Follows imports, re-exports, and aliases to the actual source.',
  schema: z.object({
  ...
  async handler(params, engine) {
    ...
    const result = await engine.request<Location | Location[] | null>('textDocument/definition', {
      textDocument: { uri },
      position,
    });
