...
export const typeHierarchy = defineTool({
  ...
  async handler(params, engine) {
    ...
    const items = await engine.request<TypeHierarchyItem[] | null>(
      'textDocument/prepareTypeHierarchy', { textDocument: { uri }, position },
    );
    if (!items || items.length === 0) return 'No type hierarchy available for this symbol.';
    const item = items[0];

    const method = params.direction === 'supertypes'
      ? 'typeHierarchy/supertypes' : 'typeHierarchy/subtypes';
    const related = await engine.request<TypeHierarchyItem[] | null>(method, { item });
