...
import { toPosition, relativePath, uriToPath, getPackageName } from '../../engine/positions.js';
import { formatHover } from '../../format/markdown.js';
import type { Location, DocumentSymbol, Hover } from 'vscode-languageserver-protocol';
...
function extractSymbolBody(content: string, startLine: number, endLine: number): string {
...
interface ContextNode {
  file: string;
  symbol: string;
  category: 'must-modify' | 'verify-only' | 'skip';
  depth: number;
  content: string;
  tokens: number;
}
...
export const gatherContext = defineTool({
  ...
  async handler(params, engine) {
    ...
    const entries: { uri: string; position: { line: number; character: number }; name: string }[] = [];
    ...
    if (entries.length === 0) return 'Error: Could not resolve any of the provided symbols.';

    // Phase 2: Build impact graph (simplified BFS)
    const visited = new Set<string>();
    const fileSymbols = new Map<string, { depth: number; symbols: string[] }>();

    for (const entry of entries) {
      const queue: { uri: string; position: { line: number; character: number }; depth: number }[] = [
    ...
    const sections = {
      mustModify: [] as string[],
      verifyOnly: [] as string[],
      skip: [] as string[],
    };
