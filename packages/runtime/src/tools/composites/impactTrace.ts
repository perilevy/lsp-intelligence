...
import { toPosition, fromPosition, relativePath, uriToPath, getPackageName } from '../../engine/positions.js';
...
export const impactTrace = defineTool({
  ...
  async handler(params, engine) {
    let uri: string, position: { line: number; character: number }, name: string;
    if (params.symbol) {
      const resolved = await engine.resolveSymbol(params.symbol, params.file_path);
      uri = resolved.uri; position = resolved.position; name = resolved.name ?? params.symbol;
    } else if (params.file_path && params.line && params.column) {
      const prepared = await engine.prepareFile(params.file_path);
      uri = prepared.uri; position = toPosition(params.line, params.column); name = 'symbol';
    } else {
      return 'Error: Provide either symbol name or file_path + line + column.';
    }

    const timeout = DEFAULT_TIMEOUTS.composite;
    const visited = new Map<string, TraceNode>();
    const queue: { uri: string; position: { line: number; character: number }; name: string; depth: number }[] = [
    ...
    for (const node of visited.values()) {
      ...
      if (node.refCount > 0) {
        if (node.depth === 0) directCallers += node.refCount;
        else if (node.isTypeAlias) typeAnnotations += node.refCount;
        else transitiveRefs += node.refCount;
      }
    }

    // Collect all unique referenced files across all depths
    const filesByDepth = new Map<string, number>();
    for (const node of visited.values()) {
    ...
    if (directCallers > 0) lines.push(`- ${directCallers} direct references`);
    if (typeAnnotations > 0) lines.push(`- ${typeAnnotations} via type aliases`);
    if (transitiveRefs > 0) lines.push(`- ${transitiveRefs} transitive (re-exports)`);
    lines.push('');
