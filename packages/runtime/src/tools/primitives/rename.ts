...
export const rename = defineTool({
  ...
  async handler(params, engine) {
    ...
    const result = await engine.request<WorkspaceEdit | null>('textDocument/rename', {
      textDocument: { uri }, position, newName: params.new_name,
    });

    if (!result || !result.changes) return 'Rename not available for this symbol.';

    const lines: string[] = [`# Rename Preview: → ${params.new_name}\n`];
    let totalEdits = 0;
