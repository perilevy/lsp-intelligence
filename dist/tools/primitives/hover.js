...
export const hover = defineTool({
    name: 'hover',
    description: 'Get the full type signature and documentation for a symbol. Accepts symbol name OR file position. Returns the same info you see when hovering in VS Code.',
    schema: z.object({
    ...
    async handler(params, engine) {
        ...
        const result = await engine.request('textDocument/hover', {
            textDocument: { uri },
            position,
        });
